/* eslint-disable */
export default {
  viewDetails: 'Afficher l’enregistrement',
  addEntity: 'Créer un nouveau contact',
  addRecord: 'Créer un nouvel enregistrement',
  call: 'Téléphone',
  text: 'Texto',
  more: 'Plus',
  mark: 'Marquer comme non lue',
  unmark: 'Marquer comme lu',
  delete: 'Supprimer',
  viewFax: 'Afficher',
  downloadFax: 'Télécharger',
  downloadVoicemail: 'Télécharger',
  copyNumber: 'Copier le numéro',
  createLog: 'Créer un journal',
  selectRecordsForAutoLog:
    'Sélectionner les enregistrements pour la journalisation automatique',
  resolveThread: 'Résoudre',
  assignToMe: 'Répondre',
  assignThread: 'Attribuer',
  reassignThread: 'Réaffecter',
  unassignThread: 'Retirer',
} as const;

// @key: @#@"viewDetails"@#@ @source: @#@"View record"@#@
// @key: @#@"addEntity"@#@ @source: @#@"Create new contact"@#@
// @key: @#@"addRecord"@#@ @source: @#@"Create new record"@#@
// @key: @#@"call"@#@ @source: @#@"Phone"@#@
// @key: @#@"text"@#@ @source: @#@"Text"@#@
// @key: @#@"more"@#@ @source: @#@"More"@#@
// @key: @#@"mark"@#@ @source: @#@"Mark as unread"@#@
// @key: @#@"unmark"@#@ @source: @#@"Mark as read"@#@
// @key: @#@"delete"@#@ @source: @#@"Delete"@#@
// @key: @#@"viewFax"@#@ @source: @#@"View"@#@
// @key: @#@"downloadFax"@#@ @source: @#@"Download"@#@
// @key: @#@"downloadVoicemail"@#@ @source: @#@"Download"@#@
// @key: @#@"copyNumber"@#@ @source: @#@"Copy number"@#@
// @key: @#@"createLog"@#@ @source: @#@"Create log"@#@
// @key: @#@"selectRecordsForAutoLog"@#@ @source: @#@"Select records for auto log"@#@
// @key: @#@"resolveThread"@#@ @source: @#@"Resolve"@#@
// @key: @#@"assignToMe"@#@ @source: @#@"Reply"@#@
// @key: @#@"assignThread"@#@ @source: @#@"Assign"@#@
// @key: @#@"reassignThread"@#@ @source: @#@"Reassign"@#@
// @key: @#@"unassignThread"@#@ @source: @#@"Unassign"@#@
