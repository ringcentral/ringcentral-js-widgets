"use strict";

require("core-js/modules/es.object.define-property.js");
Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;
/* eslint-disable */
var _default = exports["default"] = {
  viewDetails: 'Datensatz anzeigen',
  addEntity: 'Neuen Kontakt erstellen',
  addRecord: 'Neuen Datensatz erstellen',
  call: 'Telefon',
  text: 'Text',
  more: 'Mehr',
  mark: 'Als ungelesen markieren',
  unmark: 'Als gelesen markieren',
  "delete": 'Löschen',
  viewFax: 'Anzeigen',
  downloadFax: 'Herunterladen',
  downloadVoicemail: 'Herunterladen',
  copyNumber: 'Nummer kopieren',
  createLog: 'Protokoll erstellen',
  selectRecordsForAutoLog: 'Datensätze für die automatische Protokollierung auswählen',
  resolveThread: 'Lösen',
  assignToMe: 'Antworten',
  assignThread: 'Zuweisen',
  reassignThread: 'Neu zuweisen',
  unassignThread: 'Zuweisung aufheben'
}; // @key: @#@"viewDetails"@#@ @source: @#@"View record"@#@
// @key: @#@"addEntity"@#@ @source: @#@"Create new contact"@#@
// @key: @#@"addRecord"@#@ @source: @#@"Create new record"@#@
// @key: @#@"call"@#@ @source: @#@"Phone"@#@
// @key: @#@"text"@#@ @source: @#@"Text"@#@
// @key: @#@"more"@#@ @source: @#@"More"@#@
// @key: @#@"mark"@#@ @source: @#@"Mark as unread"@#@
// @key: @#@"unmark"@#@ @source: @#@"Mark as read"@#@
// @key: @#@"delete"@#@ @source: @#@"Delete"@#@
// @key: @#@"viewFax"@#@ @source: @#@"View"@#@
// @key: @#@"downloadFax"@#@ @source: @#@"Download"@#@
// @key: @#@"downloadVoicemail"@#@ @source: @#@"Download"@#@
// @key: @#@"copyNumber"@#@ @source: @#@"Copy number"@#@
// @key: @#@"createLog"@#@ @source: @#@"Create log"@#@
// @key: @#@"selectRecordsForAutoLog"@#@ @source: @#@"Select records for auto log"@#@
// @key: @#@"resolveThread"@#@ @source: @#@"Resolve"@#@
// @key: @#@"assignToMe"@#@ @source: @#@"Reply"@#@
// @key: @#@"assignThread"@#@ @source: @#@"Assign"@#@
// @key: @#@"reassignThread"@#@ @source: @#@"Reassign"@#@
// @key: @#@"unassignThread"@#@ @source: @#@"Unassign"@#@
//# sourceMappingURL=de-DE.js.map
