...
function getIntlDateTimeFormatter() {
  ...
  return function (_ref2) {
    ...
    switch (type) {
      case 'date':
        return getFormatter(locale, dateOptions).format(utcTimestamp);
      case 'time':
        return getFormatter(locale, timeOptions).format(utcTimestamp);
      default:
        return getFormatter(locale, dateTimeOptions).format(utcTimestamp);
    }
  };
}
