...
function isSearchHitContact(_ref) {
  ...
  return (// search names
    !!contact.name && contact.name.toLowerCase().includes(lowerSearch) || [contact.firstName, contact.lastName].map(function (x) {
      ...
      return !!x;
    }).join(' ').toLowerCase().includes(lowerSearch) || Array.isArray(contact.phoneNumbers) && contact.phoneNumbers.some(function (x) {
...
var isSameSite = function isSameSite(_ref5) {
   ...
   */
  if (!siteCode || !/^[0-9]+$/.test(extensionNumber) || extensionNumber[0] === '0' && extensionNumber !== '0') {
    return false;
  }
  /**
   * [multiple site only]
