...
(function () {
  ...
  var CFSimpleSip = function CFSimpleSip(options) {
    ...
    this.ua = new _sip["default"].UA({
      ...
      userAgentString: _sip["default"].C.USER_AGENT + ';' + browserUa,
      // Fixed Options
      register: true,
...
var _default = function () {
  ...
  return function (global) {
    ...
    DialGroupChangePendingNotification.prototype.processResponse = function (notification) {
      ...
      var formattedResponse = utils.buildDefaultResponse(notification);
      formattedResponse.message = 'Dial Group Change Pending notification received.';
      formattedResponse.detail = 'DialGroup switch for existing session pending until active call ends.';
      formattedResponse.agentId = utils.getText(notif, 'agent_id');
      formattedResponse.dialGroupId = utils.getText(notif, 'dial_group_id');
    ...
    DropSessionNotification.prototype.processResponse = function (notification) {
      ...
      if (transfer) {
        console.log(sessionUII + ' TRANSFER TO ' + transfer.destination + ' TERMINATED');
    ...
    EndCallNotification.prototype.processResponse = function (notification) {
      ...
      formattedResponse.recordingUrl = utils.getText(notif, 'recording_url');
      formattedResponse.dispositionTimeout = utils.getText(notif, 'disposition_timeout');
      return formattedResponse;
    };
    ...
    NewCallNotification.prototype.processResponse = function (notification) {
      ...
      newCall.allowManualInternationalTransfer = utils.getText(notif, 'allow_manual_international_transfer');
      newCall.allowDirectAgentTransfer = utils.getText(notif, 'allow_direct_agent_transfer');
      newCall.allowHangup = utils.getText(notif, 'allow_hangup');
      newCall.allowRequeue = utils.getText(notif, 'allow_requeue');
      newCall.allowEndCallForEveryone = utils.getText(notif, 'allow_endcallforeveryone');
    ...
    var AgentStateRequest = function AgentStateRequest(agentState, agentAuxState) {
      if (agentState.toUpperCase() === 'ON-BREAK' && UIModel.getInstance().agentSettings.onCall === true) {
    ...
    function _getExtensionPresenceInfo(path, queryParams) {
      ...
      var engageAccessToken = 'Bearer ' + utils.toString(UIModel.getInstance().authenticateRequest.engageAccessToken);
      var params = {
        headers: {
          'Content-Type': 'application/json'
        }
      ...
      params['queryParams'] = queryParams;
      var errorMsg = 'Error while fetching extension presence response. URL: ' + baseUrl + path;
      new HttpService(baseUrl).httpGet(path, params).then(function (response) {
        try {
          response = JSON.parse(response.response);
    ...
    LoginRequest.prototype.processResponse = function (response) {
      ...
      if (status === 'SUCCESS') {
        if (!model.agentSettings.isLoggedIn) {
          ...
          model.agentSettings.consultCall = utils.getText(resp, 'is_consultcall');
          ...
          if (model.agentSettings.updateLoginMode) {
            ...
            } else if (model.agentSettings.onCall && (model.currentCall.uii !== model.connectionSettings.activeCallUii || Lib.waitingForAddSession === true)) {
    ...
    LoginPhase1Request.prototype.processResponse = function (response) {
      ...
      if (status === 'OK') {
        if (!model.applicationSettings.isLoggedInIS) {
          ...
          model.applicationSettings.enableCallContactHistory = utils.getText(resp, 'enable_call_contact_history');
          ...
          model.agentPermissions.allowChat = utils.getText(resp, 'allow_chat') === '1'; //Agent permission

          model.agentPermissions.enableChat = utils.getText(resp, 'enable_chat') === '1'; //Account permission
    ...
    function _getSipRegistrationInfo(path, queryParams) {
      ...
      params.headers['Authorization'] = engageAccessToken;
      params['queryParams'] = queryParams;
      var errorMsg = 'Error on request get to sip registration info. URL: ' + baseUrl + path;
      new HttpService(baseUrl).httpGet(path, params).then(function (response) {
        try {
          response = JSON.parse(response.response);
    ...
    */
    ...
    ChatListRequest.prototype.processResponse = function (response) {
      ...
      formattedResponse.chatList = utils.processResponseCollection(notif, 'chat_list', 'active_chat');
      return formattedResponse;
    };
    ...
    var ChatAgentAssignFailureNotification = function ChatAgentAssignFailureNotification() {};
    /*
    ...
    ChatAgentAssignFailureNotification.prototype.processResponse = function (notification) {
      var notif = notification.ui_notification;
      var agentAssignFailure = utils.buildDefaultResponse(notification, true);
      agentAssignFailure.message = 'Received CHAT-AGENT-ASSIGN-FAILURE notification';
    ...
    ChatPresentedNotification.prototype.processResponse = function (notification) {
      ...
      if (taskId) {
        ...
        var cachedSourceList = model.applicationSettings.edSources[mainAccountId];
        var cachedLanguageList = model.applicationSettings.edLanguages[mainAccountId];
        var cachedCategoryList = model.applicationSettings.edCategories[mainAccountId];

        var sourceCache = _getEdValue(cachedSourceList, edSourceId);

        var languageCache = _getEdValue(cachedLanguageList, edLanguageId);

        var categoryCache = _getMultiEdValues(cachedCategoryList, edCategoryIds);

        var edProxyPath = 'digital-api-proxy/evMainAccount/' + mainAccountId;

        if (!sourceCache.isCached || !languageCache.isCached || !categoryCache.isCached) {
          ...
          utils.refreshAccessToken().then(function () {
            ...
            if (sourceCache.isCached) {
              ...
              sourcePromise = _buildEtrHttpRequest(edProxyPath + '/sources', authToken);
            }

            if (languageCache.isCached) {
              edLanguage = languageCache.name;
            } else {
              languagePromise = _buildEtrHttpRequest(edProxyPath + '/languages', authToken);
            }

            if (categoryCache.isCached) {
              edCategories = categoryCache.names;
            } else {
              categoriesPromise = _buildEtrHttpRequest(edProxyPath + '/categories', authToken);
            }

            if (sourcePromise) {
              sourcePromise.then(function (sourceResponse) {
                try {
                  var sourceList = JSON.parse(sourceResponse.response);
                  model.applicationSettings.edSources[mainAccountId] = sourceList; // store returned sources in memory for later use
            ...
            if (languagePromise) {
              languagePromise.then(function (languageResponse) {
                try {
                  var languageList = JSON.parse(languageResponse.response);
                  model.applicationSettings.edLanguages[mainAccountId] = languageList; // store returned languages in memory for later use
            ...
            if (categoriesPromise) {
              categoriesPromise.then(function (categoryResponse) {
                try {
                  var categoryList = JSON.parse(categoryResponse.response);
                  model.applicationSettings.edCategories[mainAccountId] = categoryList; // store returned categories in memory for later use

                  edCategories = _getMultiEdValues(categoryList, edCategoryIds).names;
                } catch (err) {
            ...
            Promise.all([sourcePromise, languagePromise, categoriesPromise]).then(function () {
              processedResponse = _buildPresentedResponse(notification, edSourceName, edSourceColor, edLanguage, edCategories);
              utils.fireCallback(instance, CALLBACK_TYPES.CHAT_PRESENTED, processedResponse);
              return processedResponse;
            }, function (err) {
              utils.logMessage(LOG_LEVELS.WARN, 'Error resolving ETR API promises', err);
              processedResponse = _buildPresentedResponse(notification, edSourceName, edSourceColor, edLanguage, edCategories);
              utils.fireCallback(instance, CALLBACK_TYPES.CHAT_PRESENTED, processedResponse);
              return processedResponse;
            });
    ...
    function _buildPresentedResponse(notification, edSourceName, edSourceColor, edLanguage, edCategories) {
      var notif = notification.ui_notification;
      var formattedResponse = utils.buildDefaultResponse(notification, true);
      ...
      formattedResponse.edAuthorScreenName = utils.getText(notif, 'ed_author_screen_name');
    ...
    var LoadMediaNotification = function LoadMediaNotification() {};
    /*
     * Engage Digital Chat:
    ...
    LoadMediaNotification.prototype.processResponse = function (notification) {
      ...
      newTask.requeueShortcuts = utils.processResponseCollection(notification, 'ui_notification', 'chat_requeue_shortcuts', 'shortcut')[0];
      newTask.chatDispositions = utils.processResponseCollection(notification, 'ui_notification', 'chat_dispositions', 'disposition')[0];
    ...
    */
    ...
    var UIModel = function () {
      ...
      function init() {
        ...
        return {
          ...
          baseEtrUri: '/platform/api/ed-task-routing/v1/',
          // the path to ETR task routing endpoints
          ...
          applicationSettings: {
            ...
            isFlr: false,
            // Check of Force legacy registration for dialing destination type
            dialDestType: '',
            ...
            allowMultiSocket: false,
            // Determines whether agent can open a new socket under the same login
            edSources: {},
            // Cache for ED sources, loaded when first task received. Object indexed by mainAccountId, then array of all sources
            edLanguages: {},
            // Cache for ED language, loaded when first task received. Object indexed by mainAccountId, then array of all languages
          ...
          agentPermissions: {
            ...
            enableChat: false,
            // Controls whether or not the agent has legacy chat and SMS access on the account level
          ...
          softphoneSettings: {
            ...
            // set to true when attempting to rotate softphone registrar and reconnect
    ...
    LocalStorageService.prototype.get = function (key) {
      // TODO: that window.localStorage is add by ringcentral-integration
      if (!window.localStorage || !key || !window.localStorage.getItem(this.prefix + key)) {
    ...
    var utils = {
      ...
      processResponse: function processResponse(instance, response) {
        ...
        var message = 'Received ' + type.toUpperCase() + ' response message from ' + dest; // log message response
      ...
      processNotification: function processNotification(instance, data) {
        ...
        var message = 'Received ' + type.toUpperCase() + ' notification message from ' + dest; // log message response
        ...
        switch (type.toUpperCase()) {
          ...
          case MESSAGE_TYPES.CHAT_LOAD_MEDIA:
            ...
            utils.fireCallback(instance, CALLBACK_TYPES.CHAT_LOAD_MEDIA, loadMediaResp);
            break;

          case MESSAGE_TYPES.CHAT_AGENT_ASSIGN_FAILURE:
            var chatAgentAssignFailureNotif = new ChatAgentAssignFailureNotification();
            var chatAgentAssignFailureResp = chatAgentAssignFailureNotif.processResponse(data);
            utils.fireCallback(instance, CALLBACK_TYPES.CHAT_AGENT_ASSIGN_FAILURE, chatAgentAssignFailureResp);
            break;
          ...
          case MESSAGE_TYPES.LOGOUT:
            ...
            var model = UIModel.getInstance(); //remove agent_id and hash_code from local storage

            model.dataStore.remove('agent_id');
            model.dataStore.remove('hash_code'); // set login flag here in case notification initiated externally
      ...
      processDialerResponse: function processDialerResponse(instance, response) {
        ...
        var dest = messageId === '' ? 'IS' : messageId.slice(0, 2);
        var message = 'Received ' + type.toUpperCase() + ' dialer response message from ' + dest; // log message response
      ...
      processStats: function processStats(instance, data) {
        ...
        var message = 'Received ' + type.toUpperCase() + ' response message from IS'; // Fire callback function
      ...
      buildDefaultResponse: function buildDefaultResponse(response, includeOpId) {
        ...
        var det = '';
        var stat = '';
        var opId = ''; // add properties if present
      ...
      getDialDestination: function getDialDestination() {
        ...
        if (softphoneSettings.sipInfo.length > 0) {
          ...
          if (dialDestType === 'LEGACY_SOFTPHONE') {
            dialDest = 'sip:' + utils.escapeSoftphoneUsername(currentServer.username) + '@' + currentServer.domain;
          } else if (dialDestType === 'RC_SOFTPHONE') {
            dialDest = utils.escapeSoftphoneUsername(currentServer.username) + '@RC_SOFTPHONE';
          }
        }
      ...
      refreshAccessToken: function refreshAccessToken() {
        ...
        return new HttpService(baseUrl).httpPost('login/agent/hashcode', params).then(function (response) {
          try {
            response = JSON.parse(response.response);
      ...
      generateOpId: function generateOpId(length) {
        length = length || 24;
    ...
    var CALLBACK_TYPES = {
      ...
      // external chat
      CHAT_LOAD_MEDIA: 'chatLoadMediaResponse',
      // engage digital task
      CHAT_AGENT_ASSIGN_FAILURE: 'chatAgentAssignFailureResponse',
      // engage digital task
    ...
    var MESSAGE_TYPES = {
      ...
      // external chat
      CHAT_LOAD_MEDIA: 'LOAD-MEDIA',
      // engage digital task
      CHAT_AGENT_ASSIGN_FAILURE: 'CHAT-AGENT-ASSIGN-FAILURE',
      // engage digital task
    ...
    function initAgentLibrarySoftphoneService(context) {
      ...
      var AgentLibrary = context.AgentLibrary; // var agentAuthRequest = UIModel.getInstance().authenticateRequest;
      ...
      var SoftphoneService = {
        setupWebRtcServer: function setupWebRtcServer() {
          ...
          if (sipInfo !== null) {
            ...
            softphoneSettings.uri = utils.escapeSoftphoneUsername(username) + '@' + sipInfo.domain;
      ...
      function _rotateWebRtcServer(callback) {
        var softphoneSettings = UIModel.getInstance().softphoneSettings;
        utils.setCallback(UIModel.getInstance().libraryInstance, CALLBACK_TYPES.ROTATE_WEBRTC_SERVER, callback);
        var sipInfo = softphoneSettings.sipInfo;
        var timeSinceLastFetch = Date.now() - softphoneSettings.lastSipFetchTime;
        console.log('## Checking to rotate webRtc Server.SipInfo array length is ' + sipInfo.length + '. Time since last fetch of webRtcInfo is ' + timeSinceLastFetch / 60000 + ' mins.');

        if (sipInfo.length > 1 && timeSinceLastFetch < 5 * 60000) {
          sipInfo.shift();
          console.log('## rotated sip server.Will now attempt to new server: ' + sipInfo[0].domain);
          softphoneSettings.attemptingSoftphoneReconnect = true;
          utils.fireCallback(UIModel.getInstance().libraryInstance, CALLBACK_TYPES.ROTATE_WEBRTC_SERVER, callback);
        } else {
          console.log('## fetching webRtcInfo.'); // fetch Sip Registration info again if array is consumed to rotate registrar
      ...
      function _ringing(notif) {
        var model = UIModel.getInstance();
        var usingRcPhone = model.agentSettings.dialDest.indexOf('@RC_SOFTPHONE') > -1;
        var rcHeader = notif.transaction && notif.transaction.request && notif.transaction.request.headers['P-Rc-Call-Source'];
        var rcHeaderValue = rcHeader && rcHeader.length === 1 && rcHeader[0].raw; // do not notify the app of ringing if Rc Call Source is not 'partner-contact-center'
