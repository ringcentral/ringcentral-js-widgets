...
exports.toPseudoString = toPseudoString;
exports.toPseudoStringWithPadding = toPseudoStringWithPadding;
exports.toHashPseudoString = toHashPseudoString;
...
const padCharacters = '~!@#$%^&*';
const replaceFunctions = Object.keys(exports.charMap).map((char) => {
    const regExp = new RegExp(char, 'g');
    const accentChar = String.fromCharCode(exports.charMap[char]);
 ...
 * ```
 */
function padString({ str, padRatio = 0.3, padChar = padCharacters, } = {}) {
 ...
 * ```
 */
function toPseudoStringWithPadding({ str, padRatio, padChar = padCharacters, }) {
 ...
 * @returns
 */
function generateHash(filePath, keyPath) {
    return (0, md5_1.default)(`${filePath}_${keyPath}`).toString().substring(0, 8);
}
function toHashPseudoString(filePath, keyPath, value) {
