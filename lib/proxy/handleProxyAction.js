...
var dropStates = function dropStates(action) {
  var states = action.states,
      actionRest = _objectWithoutProperties(action, ["states"]);
...
var pushStates = function pushStates(target, action) {
  if (Array.isArray(action.type) && Array.isArray(action.patches) && typeof action.__name__ === 'string') {
    // restore changes states for reduction of serialized data from `patches`
    var states = (0, _core.applyPatches)(target[action.__name__].state, action.patches);
