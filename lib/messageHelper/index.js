...
exports.getMMSAttachment = getMMSAttachment;
...
function messageIsTextMessage(message) {
  return message.type !== _messageTypes.default.fax && message.type !== _messageTypes.default.voiceMail;
}

function messageIsFax(message) {
  return message.type === _messageTypes.default.fax;
}

function messageIsVoicemail(message) {
  return message.type === _messageTypes.default.voiceMail;
}

function messageIsAcceptable(message) {
  // do not show submitted faxes or sending failed faxes now
  // do not show deleted messages
  return (message.type !== _messageTypes.default.fax || message.messageStatus !== 'Queued' && message.messageStatus !== 'SendingFailed') && !messageIsDeleted(message);
}

function getMessageType(message) {
  if (messageIsTextMessage(message)) {
    return _messageTypes.default.text;
  } else if (messageIsVoicemail(message)) {
    return _messageTypes.default.voiceMail;
  } else if (messageIsFax(message)) {
    return _messageTypes.default.fax;
  }
...
function getMyNumberFromMessage(_ref) {
  ...
  if (message.type === _messageTypes.default.pager) {
...
function getRecipientNumbersFromMessage(_ref2) {
  ...
  if (message.type === _messageTypes.default.sms) {
    if (message.direction === 'Outbound') {
      return message.to;
    }
...
function getMMSAttachment(message, accessToken) {
...
function normalizeRecord(record) {
  ...
  return _objectSpread({}, newRecord, {
