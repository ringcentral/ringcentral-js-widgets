...
exports.renderContactName = renderContactName;
    ...
    if (outboundIndex > -1) {
      ...
      if (inbound.action && outbound.action) {
        ...
        var call = (0, _extends3.default)({}, outbound, {
          ...
          inboundLeg: inbound,
          from: (0, _extends3.default)({}, inbound.from),
          to: (0, _extends3.default)({}, inbound.to),
...
function getName() {
  var _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {},
      _ref$nameEntities = _ref.nameEntities,
      nameEntities = _ref$nameEntities === undefined ? [] : _ref$nameEntities,
  ...
  var selectFieldNames = {
    anonymous: _i18n2.default.getString('anonymous', currentLocale),
    nameUnknown: _i18n2.default.getString('nameUnknown', currentLocale),
    nameMultiple: _i18n2.default.getString('nameMultiple', currentLocale)
  };
  if (anonymous) {
    return selectFieldNames.anonymous;
  }
  var unknownDisplayText = typeof normalizeNumber === 'undefined' ? selectFieldNames.nameUnknown : normalizeNumber;

  if (!nameEntities) {
    return unknownDisplayText;
  }
  var nameValidEntities = nameEntities.filter(function (entities) {
    return entities && entities.id;
  });
  if (nameValidEntities.length === 0) {
    return unknownDisplayText;
  }
  var isMultiple = nameValidEntities.length > 1;
  return isMultiple ? normalizeNumber || selectFieldNames.nameMultiple : nameValidEntities[0].name;
}
// Get phone number and matches.
function getPhoneNumberMatches() {
  var call = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var _call$to = call.to,
      to = _call$to === undefined ? {} : _call$to,
      _call$from = call.from,
      from = _call$from === undefined ? {} : _call$from,
...
function renderContactName(call, currentLocale) {
  var _getPhoneNumberMatche = getPhoneNumberMatches(call),
