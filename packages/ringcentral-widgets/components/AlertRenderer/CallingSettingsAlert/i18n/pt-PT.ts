...
export default {
  ...
  [callingSettingsMessages.saveSuccessWithSoftphone]:
    'Definições guardadas com sucesso. Certifique-se de que tem o {brand} instalado no computador.',
  [callingSettingsMessages.permissionChanged]:
    'As suas permissões foram alteradas recentemente. Aceda a {link} para verificar as opções de Chamadas.',
  [callingSettingsMessages.phoneNumberChanged]:
    'As informações do seu número de telefone foram alteradas recentemente. Aceda a {link} para verificar as opções de Chamadas.',
  link: 'Definições > Chamadas',
  [callingSettingsMessages.webphonePermissionRemoved]:
    'As suas permissões foram alteradas e não pode efetuar chamadas com o browser. Para obter detalhes, contacte o administrador da conta.',
  [callingSettingsMessages.emergencyCallingNotAvailable]:
    'A marcação de números de emergência ou de serviços especiais não é suportada. Em caso de emergência, utilize o telefone fixo ou o telemóvel para ligar para um número de emergência.',
