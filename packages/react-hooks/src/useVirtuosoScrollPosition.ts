...
export const useVirtuosoScrollPosition = (
  onScroll: (snapshot: StateSnapshot) => void,
  delay: number = 300,
): {
  ...
  const virtuosoActionsRef = useRef<VirtuosoHandle>(null);

  const handleScroll = useDebounce(
    () => {
      virtuosoActionsRef.current?.getState((snapshot) => {
