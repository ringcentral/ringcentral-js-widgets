...
describe('RCI-330 - Authorize and Unauthorize RingCentral for Office 365', () => {
  test.skip({
    ...
    await $(page).waitFor('div[title="Schedule Meeting"]', {
      selector: 'css'
    });
    await $(page).click('div[title="Schedule Meeting"]', {
      selector: 'css'
    });
     ...
     * 2. Authorize button
     **/
    const buttonText = await $(page).getText('button[class*="widgets-components-MeetingScheduleButton-_styles_button"]', {
      selector: 'css'
    });
    expect(buttonText).toEqual(option.buttonText);
    const actionPrompt = await $(page).getText("div[class*='styles_actionPrompt']", {
      selector: 'css'
    });
     ...
     * check auth panel
     */
    await $(page).click("button[class*='widgets-components-MeetingScheduleButton-_styles_button']", {
      selector: 'css'
    });
    await goThrough3rdPartyLogin();
    /** New meeting should be displayed */
    await $(page).waitFor(2000);
    const inviteText = await $(page).getText("div[class*='styles_inviteBox'] > button", {
      selector: 'css'
    });
  ...
  test.skip({
    ...
    await $(page).waitFor('div[title="Schedule Conference"]', {
      selector: 'css'
    });
    await $(page).click('div[title="Schedule Conference"]', {
      selector: 'css'
    });
    await $(page).waitFor('div[class*="styles_bottom"]>div[class*="ringcentral-widgets-components-Button"]', {
      selector: 'css'
    });
    await $(page).click('div[class*="styles_bottom"]>div[class*="ringcentral-widgets-components-Button"]', {
      selector: 'css'
    });
    await $(page).click('div[class*="styles_bottom"]>div[class*="ringcentral-widgets-components-Button"]', {
      selector: 'css'
    });
    await $(page).click("a[class*='styles_clickHere']", {
      selector: 'css'
    });
    const targets = await browser.targets();
    const officeAuthPage = await targets[targets.length - 1].page();
    await goThrough3rdPartyLogin(officeAuthPage);
    const alertMessage = await $(page).getText('div[class*="ringcentral-widgets-components-AlertDisplay"]', {
      selector: 'css'
    });
