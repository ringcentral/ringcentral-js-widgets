...
describe('test: =====>', () => {
  ...
  test({
    ...
    // const text = await browser.text(option.selector);
    // expect(text).toBe(option.expected);
    const app = await getWrapper();
    console.log(browser.program.mount(app).find('[data-sign="loginButton"]').first().text());
    // expect(browser.program.mount(<Button>ttt</Button>).text()).toBe('ttt');
  });
});
