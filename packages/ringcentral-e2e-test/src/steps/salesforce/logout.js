export default class Settings {
  ...
  static async logout({ options: { option, isVirtual }, driver: { app } }) {
    if (isVirtual) {
      app.props().phone.auth.logout({ username: option.account[0]['mainNumber'], password: option.account[0]['password']});
    } else {
      await $(app).execute(`phone.auth.logout({username: '${option.account[0]['mainNumber']}', password: '${option.account[0]['password']}'})`);
    }
    await $(app).waitFor('[class*=loginButton]',{ selector: 'css' });
  }
