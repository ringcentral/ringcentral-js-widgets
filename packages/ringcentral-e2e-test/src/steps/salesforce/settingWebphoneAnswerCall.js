...
export default class WebphoneAnserCall {
  ...
  static async setting2PhoneAnswerCall(context) {
    ...
    let accountLength = context.options.option.accounts.length;
    let webphone1 = await webphone.createWebPhone('+' + context.options.option.accounts[accountLength - 2]['did'], 'webphone', 'Test!123');
    let result1 = await webphone.preOperateAnswerCall(JSON.parse(webphone1.text)._id, JSON.parse(webphone1.text).sessionId);
    let webphone2 = await webphone.createWebPhone('+' + context.options.option.accounts[accountLength - 1]['did'], 'webphone', 'Test!123');
