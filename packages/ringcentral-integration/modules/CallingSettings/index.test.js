...
describe('CallingSettings Unit Test', () => {
  ...
  describe('_shouldValidate', () => {
    runBooleanTest(
      (
        ...
        it(`should return ${result} when:
          callingSettings.ready is ${ready}
          callingSettings._ringoutEnabled is ${
            ringoutEnabledChanged ? 'not' : ''
          }
          equal to callingSettings._rolesAndPermissions.ringoutEnabled and
          callingSettings._webphoneEnabled is ${
            webphoneEnabledChanged ? 'not' : ''
          }
          equal to callingSettings._rolesAndPermissions.webphoneEnabled and
          callingSettings._myPhoneNumbers is ${
            myPhoneNumbersChanged ? 'not' : ''
          }
          equal to callingSettings.myPhoneNumbers and
          callingSettings._otherPhoneNumbers is ${
            myPhoneNumbersChanged ? 'not' : ''
          }
          ...
          sinon.stub(callingSettings, 'myPhoneNumbers', { get: () => '456' });
          sinon.stub(callingSettings, 'otherPhoneNumbers', {
            get: () => '456',
          });
  ...
  describe('_validateSettings', () => {
    ...
    when _hasWebphonePermissionRemoved is true`, async () => {
      sinon
        .stub(callingSettings, '_hasWebphonePermissionRemoved')
        .callsFake(() => true);
      sinon
        .stub(callingSettings, '_hasPermissionChanged')
        .callsFake(() => false);
      sinon
        .stub(callingSettings, '_hasPhoneNumberChanged')
        .callsFake(() => false);
