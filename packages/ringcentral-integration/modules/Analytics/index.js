...
export default class Analytics extends RcModule {
  ...
  _allCallsClickHangup(action) {
    if (this._router
      && this._router.currentPath === '/calls'
      && this._callMonitor
  ...
  _callsOnHoldClickHangup(action) {
    if (this._router
      && this._router.currentPath.includes('/conferenceCall/callsOnhold')
      && this._callMonitor
      && this._callMonitor.actionTypes.allCallsClickHangupTrack === action.type
    ) {
      this.track('Click Hangup (Calls On Hold)');
    }
  }

  _callsOnHoldClickAdd(action) {
    if (this._conferenceCall
      && this._conferenceCall.actionTypes.callsOnHoldClickAddTrack === action.type
    ) {
      this.track('Click Add (Calls On Hold)');
    }
  }

  _callsOnHoldClickMerge(action) {
    if (this._conferenceCall
      && this._conferenceCall.actionTypes.callsOnHoldClickMergeTrack === action.type
    ) {
      this.track('Click Merge (Calls On Hold)');
    }
  }

  _confirmMergeClickClose(action) {
    if (this._conferenceCall
      && this._conferenceCall.actionTypes.confirmMergeClickCloseTrack === action.type
    ) {
  ...
  _confirmMergeClickMerge(action) {
    if (this._conferenceCall
      && this._conferenceCall.actionTypes.confirmMergeClickMergeTrack === action.type
    ) {
