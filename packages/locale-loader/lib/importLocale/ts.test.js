...
const sourceFolder = './testData/importLocale2';
const localizationFolder = './localization/importLocale2';
...
async function generateSource() {
  ...
  await fs.writeFile(
    path.resolve(sourceFolder, 'loadLocale.ts'),
    '/* loadLocale */',
  );
  await fs.writeFile(
    path.resolve(sourceFolder, 'en-US.ts'),
    `
    const obj = {
      key: 'testKey',
    };
...
describe('importLocale', () => {
  ...
  test('should import generated xlf files', async () => {
    ...
    const filePath = path.resolve(sourceFolder, 'en-GB.ts');
    expect(await fs.exists(filePath)).toBe(true);
    const content = await fs.readFile(filePath, 'utf8');
    let json;
    expect(() => {
      json = eval(transformSync(content, { filename: 'en-GB.ts' }).code);
    }).not.toThrow();
  ...
  test('it should work for files without trailing comma', async () => {
    ...
    const content = await fs.readFile(filePath, 'utf8');
    expect(() => {
      eval(transformSync(content, { filename: 'en-GB.ts' }).code);
    }).not.toThrow();
  });
});
