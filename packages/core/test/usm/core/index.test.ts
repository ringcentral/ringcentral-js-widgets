import Module from '../../../lib/usm/core/module';
import moduleStatuses from '../../../lib/usm/core/moduleStatuses';
...
test('create instance default params', () => {
  ...
  expect(typeof Object.getOwnPropertyDescriptor(foo, 'getState').value).toEqual(
    'function',
  );
});
...
test('single module lifecycle', async () => {
  ...
  const foo = new Foo();
  foo.bootstrap();
  expect(spy.mock.calls).toEqual([['moduleWillInitialize']]);
  ...
  foo.resetModule();
  expect(spy.mock.calls.slice(length)).toEqual([['moduleWillReset']]);
  length = spy.mock.calls.length;
  await sleep();
  expect(spy.mock.calls.slice(length)).toEqual([
...
test('multi-modules lifecycle', async () => {
  ...
  length = spy.mock.calls.length;
  fooBar.resetModule();
  expect(spy.mock.calls.slice(length)).toEqual([['Bar moduleWillReset']]);
  length = spy.mock.calls.length;
  await sleep();
  expect(spy.mock.calls.slice(length)).toEqual([
    ['Bar moduleWillInitialize'],
