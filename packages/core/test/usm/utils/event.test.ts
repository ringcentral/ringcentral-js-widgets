import event from '../../../lib/usm/utils/event';
...
test('event `on` & `emit`', () => {
  ...
  expect(spy.mock.calls).toEqual([['on callback', 'foo', 'bar']]);
});
...
test('event `on`, `emit` & `remove`', () => {
  ...
  try {
    ...
    expect(e.toString()).toEqual(
      "Error: Event type 'test' should be registered before emit it.",
    );
  }
});
...
test('event `on`, `emit` & `off`', () => {
  ...
  try {
    ...
    expect(e.toString()).toEqual(
      "Error: Event type 'test' has not any listener.",
    );
  }
  event.on('test', callback);
  event.emit('test', 'foo', 'bar');
...
test('event `on` about duplication', () => {
  ...
  try {
    ...
    expect(e.toString()).toEqual(
      "Error: Event type 'test' has been registered, please re-register it.",
    );
  }
});
...
test('event `emit` with multi-listeners include remove inner-listener function', () => {
  ...
  event.on('test', callback1);
  event.emit('test', 'foo', 'bar');
  expect(spy.mock.calls).toEqual([['on callback1', 'foo', 'bar']]);
  event.emit('test', 'foo', 'bar');
  expect(spy.mock.calls).toEqual([['on callback1', 'foo', 'bar']]);
});
