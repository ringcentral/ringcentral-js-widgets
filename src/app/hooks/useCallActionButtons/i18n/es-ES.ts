/* eslint-disable */
export default {
  unmute: 'Reactivar',
  mute: 'Silenciar',
  keypad: 'Teclado',
  audio: 'Audio',
  hold: 'Retener',
  unHold: 'Reanudar',
  park: 'Aparcar',
  stopRecord: 'Detener grabación',
  record: 'Grabar',
  add: 'Añadir',
  transfer: 'Transferir',
  flip: 'Transferir',
  mergeToConference: 'Combinar',
  mergeTooltip: 'Combinar con la llamada activa',
  end: 'Fin',
  endCall: 'Finalizar llamada',
  leaveCall: 'Abandonar llamada',
  voicemail: 'A buzón de voz',
  callList: 'Llamadas',
  ignore: 'Ignorar',
  answer: 'Responder',
  forward: 'Desviar',
  reply: 'Responder',
  endAndAnswer: 'Finalizar y responder',
  holdAndAnswer: 'En espera y responder',
  swap: 'Intercambiar llamadas',
  switch: 'Cambiar',
  notes: 'Notas',
  stopNotes: 'Detener notas',
} as const;

// @key: @#@"unmute"@#@ @source: @#@"Unmute"@#@
// @key: @#@"mute"@#@ @source: @#@"Mute"@#@
// @key: @#@"keypad"@#@ @source: @#@"Keypad"@#@
// @key: @#@"audio"@#@ @source: @#@"Audio"@#@
// @key: @#@"hold"@#@ @source: @#@"Hold"@#@
// @key: @#@"unHold"@#@ @source: @#@"Unhold"@#@
// @key: @#@"park"@#@ @source: @#@"Park"@#@
// @key: @#@"stopRecord"@#@ @source: @#@"Stop Rec"@#@
// @key: @#@"record"@#@ @source: @#@"Record"@#@
// @key: @#@"add"@#@ @source: @#@"Add"@#@
// @key: @#@"transfer"@#@ @source: @#@"Transfer"@#@
// @key: @#@"flip"@#@ @source: @#@"Flip"@#@
// @key: @#@"mergeToConference"@#@ @source: @#@"Merge"@#@
// @key: @#@"mergeTooltip"@#@ @source: @#@"Merge with active call"@#@
// @key: @#@"end"@#@ @source: @#@"End"@#@
// @key: @#@"endCall"@#@ @source: @#@"End call"@#@
// @key: @#@"leaveCall"@#@ @source: @#@"Leave call"@#@
// @key: @#@"voicemail"@#@ @source: @#@"To voicemail"@#@
// @key: @#@"callList"@#@ @source: @#@"Calls"@#@
// @key: @#@"ignore"@#@ @source: @#@"Ignore"@#@
// @key: @#@"answer"@#@ @source: @#@"Answer"@#@
// @key: @#@"forward"@#@ @source: @#@"Forward"@#@
// @key: @#@"reply"@#@ @source: @#@"Reply"@#@
// @key: @#@"endAndAnswer"@#@ @source: @#@"End & Answer"@#@
// @key: @#@"holdAndAnswer"@#@ @source: @#@"Hold & Answer"@#@
// @key: @#@"swap"@#@ @source: @#@"Swap calls"@#@
// @key: @#@"switch"@#@ @source: @#@"Switch"@#@
// @key: @#@"notes"@#@ @source: @#@"Notes"@#@
// @key: @#@"stopNotes"@#@ @source: @#@"Stop notes"@#@
