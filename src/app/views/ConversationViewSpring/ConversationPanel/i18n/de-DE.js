"use strict";

require("core-js/modules/es.object.define-property.js");
Object.defineProperty(exports, "__esModule", {
  value: true
});
exports["default"] = void 0;
/* eslint-disable */
var _default = exports["default"] = {
  dncAlert: 'Dieser Kontakt ist auf einer "Nicht anrufen"-Liste.',
  optOutAlert: 'Dieser Kontakt hat den Empfang von Nachrichten deaktiviert.',
  smsCapabilityAlert: 'Ihre Nummer {phoneNumber} muss registriert sein, damit Sie Textnachrichten senden und empfangen können. Alternativ können Sie von einer registrierten Nummer aus auf die Textnachricht antworten.',
  saveLog: 'Protokoll speichern',
  viewInCrm: 'Anzeigen auf {crmName}',
  createLog: 'Protokoll erstellen',
  insertOptOut: 'Abmeldung einfügen',
  removeOptOut: 'Abmeldung entfernen'
}; // @key: @#@"dncAlert"@#@ @source: @#@"This contact is on a Do Not Call list."@#@
// @key: @#@"optOutAlert"@#@ @source: @#@"The recipient has opted out."@#@
// @key: @#@"smsCapabilityAlert"@#@ @source: @#@"Your number {phoneNumber} must be registered to send and receive texts. Alternatively, you can reply to the text from a registered number."@#@
// @key: @#@"saveLog"@#@ @source: @#@"Save log"@#@
// @key: @#@"viewInCrm"@#@ @source: @#@"View in {crmName}"@#@
// @key: @#@"createLog"@#@ @source: @#@"Create log"@#@
// @key: @#@"insertOptOut"@#@ @source: @#@"Insert opt-out"@#@
// @key: @#@"removeOptOut"@#@ @source: @#@"Remove opt-out"@#@
//# sourceMappingURL=de-DE.js.map
